...
const { makeMarker, findAllMap } = require('../../../models/map/FindMap');
const { makeCluster } = require('../../../modules/makeCluster-md');

router.get('/sm', async (req, res, next) => {
  try {
    let { top, bottom, left, right, sido, sigungu, dong, item_type } = req.query;
    let _top = Number(top);
    let _bottom = Number(bottom);
    let _left = Number(left);
    let _right = Number(right);
    const { rs } = await makeMarker(top, bottom, left, right, sido, sigungu, dong, item_type);
